...
function M.cmp_source(name, icon)
  icon = icon or Config.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.root_dir(opts)
  ...
  local function get()
    local cwd = _G.Config.root.cwd()
    local root = _G.Config.root.get({ normalize = true })
    local name = vim.fs.basename(root)

    if root == cwd then
      return opts.cwd and name or false
    elseif root:find(cwd, 1, true) == 1 then
      return opts.subdirectory and name or false
    elseif cwd:find(root, 1, true) == 1 then
      return opts.parent and name or false
    else
      return opts.other and name or false
    end
  end
